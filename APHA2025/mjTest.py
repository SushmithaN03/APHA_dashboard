...
#     data = apha_followers_count_analysis(preApha=True)
#     print("APHA followers_count_analysis:")
#     print(data)
#========================================================================================================================================================================
...
#========================================================================================================================================================================
# function to plot the Articles views graph for cannes website
#========================================================================================================================================================================

def apha_2025_Articles_Analysis_Graph(preApha=None, postApha=None, Apha=None):
    date_or_range = get_date_or_range(preApha, postApha, Apha)

    # If the return value is a tuple (like in the cannes case), unpack it
    if isinstance(date_or_range, tuple):
    ...
    try:
        ...
        if connection.is_connected():
            ...
            fig.update_layout(
                title={
                    #'text': f'Cannes Total Articles Views {start_date} to {end_date}',
                    'x': 0.5,
                    'xanchor': 'center',
                    'font': {'size': 12}
                },
...
def get_selected_columns_in_date_range_FBPost(preApha=False, postApha=False, Apha=None):
    ...
    if isinstance(date_range, tuple):  # For preCannes or postCannes
        if " to " in date_range[0]:  # Check if " to " exists
            start_date, end_date = date_range[0].split(" to ")
        else:
            start_date = date_range[0]
            end_date = None
    elif isinstance(date_range, list):  # For cannes-specific dates
        start_date = date_range[0]
        end_date = None
...
# if __name__ == "__main__":
#     results, df = get_selected_columns_in_date_range_FBPost(preApha=True)
    
#     # Print DataFrame
#     print("\n📊 Facebook Top Posts DataFrame:\n", df)
...
# ---- Call the function at the end ----
# if __name__ == "__main__":
#     results, df = get_selected_columns_in_date_range_InstaPost(preApha=True)
...
# ---- Call the function at the end ----
# if __name__ == "__main__":
#     results = get_selected_YoutubeVideosHealth(preApha=True)
    
#     # Print DataFrame-like info
#     if "top_posts" in results:
#         print("\n📊 YouTube Top Posts JSON:\n", results["top_posts"])
...
# ---- Call the function at the end ----
# if __name__ == "__main__":
#     results = apha_Articles_Analysis(preApha=True)
#     print(results)
...
def coreFollowers(preApha=False, postApha=False, Apha=None):
    ...
    if isinstance(date_range, tuple):  # For preCannes, postCannes, or specific Cannes date
        start_date = date_range[0]
    ...
    try:
        if connection.is_connected():
            ...
            df['Followers'] = df['Followers'].apply(
                lambda x: "{:,}".format(int(x.replace(',', '').replace('.', ''))) if isinstance(x, str) and x.replace(',', '').replace('.', '').isdigit() else x
            )
...
if __name__ == "__main__":
    # Step 1: Fetch Facebook and Instagram DataFrames for preApha period
    # (replace with your actual data-fetching functions)
    fb_results, hashtag_fb_df = get_selected_columns_in_date_range_FBPost(preApha=True)
    insta_results, hashtag_insta_df = get_selected_columns_in_date_range_InstaPost(preApha=True)

    # Step 2: Run hashtag analysis
    results = hashtag_analysis(hashtag_fb_df, hashtag_insta_df)

    # Step 3: Print both DataFrames
    print("\n📊 Facebook Hashtag DataFrame:")
    print(pd.DataFrame(results['fb']['data']))

    print("\n📊 Instagram Hashtag DataFrame:")
    print(pd.DataFrame(results['insta']['data']))
...
# Print the data
# if __name__ == "__main__":
#     data = current_followers_data()
#     print("Current Followers Data:")
#     print(data)
#========================================================================================================================================================================
...
def apha_followers_cumulative_analysis(preApha=False, postApha=False, Apha=None):
    ...
    # Case 1: Pre/post Cannes with "to" range
...
##################################################################################################################################################################
# DOD FOLLOWERS COMPARSION CANNES 2024 VS CANNES 2025
#########################################################################################################################################################################
...
def dod_followers_change_count(preApha=None, postApha=None, Apha=None):
    ...
    if preApha or postApha:
        # For pre-Cannes or post-Cannes, we need to extract all days in the range
        ...
        # For specific Cannes days, generate day by day
        def generate_day_ranges(n):
            date_ranges = []
            for day in range(1, n + 1):
...
def plot_apha_dod_followers_comparison_graph_(df, preApha=None, postApha=None, Apha=None):
    ...
    for platform in unique_platforms:
        ...
        # output_filename = f"{platform}_followers_{'preCannes' if preCannes else 'postCannes' if postCannes else 'cannes'}.jpeg"
        # fig.write_image(output_filename)
        # fig.show()
